...
block C = roundrectangle("C", (1, 2), minwidth=3cm, minheight=0.8cm);
...
add(new void(picture pic, transform t) {
  ...
  A{0.2} -- Label("0.2", 0, W) -- Right -- Up -- B{0.8}
    -- Up -- Label("?", 1, N) --- C -- Up -- Arrow --- D;
  // from block D to the extension of the right edge of block A
  D{0.5} -- Left -- Label("0.5", 0, E) -- Arrow --- A;
});
```
...
### Complex examples
```cpp {cmd=env args=[asyco -f png -M 1mm -render 4 --img-zoom=0.25x] output=html .hide}
import flowchart_mod;
